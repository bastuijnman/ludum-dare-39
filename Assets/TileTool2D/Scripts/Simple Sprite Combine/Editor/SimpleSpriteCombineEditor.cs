...
public class SimpleSpriteCombineEditor : Editor {
	...
	public void CombineSprites() {
		...
		holder.transform.position = sTarget.transform.position;
		holder.transform.rotation = sTarget.transform.rotation;
		holder.transform.localScale = sTarget.transform.localScale;
		holder.transform.parent = sTarget.transform;
		SpriteRenderer[] renderers = FindSpriteRenderers();
		List<Material> materials = new List<Material>();
		for (int i = 0; i < renderers.Length; i++) {
			if (renderers[i].sprite == null) continue;
			GameObject go = new GameObject();
			go.transform.position = renderers[i].transform.position;
			go.transform.localScale = renderers[i].transform.localScale;
			go.transform.rotation = renderers[i].transform.rotation;
			go.transform.parent = holder.transform;
			MeshFilter mf = go.AddComponent<MeshFilter>();
			mf.sharedMesh = ConvertSprite(renderers[i]);
			MeshRenderer mr = go.AddComponent<MeshRenderer>();
			for (int j = 0; j < materials.Count; j++) {
				if (renderers[i].sprite.texture == materials[j].mainTexture) {
					mr.sharedMaterial = materials[j];
					break;
				}
			}
			if(mr.sharedMaterial == null) {
				...
				materials.Add(m);
				m.name = sTarget.transform.name + " material (" + materials.Count.ToString()+ ")";
				m.mainTexture = renderers[i].sprite.texture;
				mr.material = m;
			}
		}
		CombineMeshes();
	...
	public MeshFilter[] FindEnabledMeshes() {
		...
		int count = 0;
		renderers = sTarget.transform.GetComponentsInChildren<MeshFilter>();
	...
	public void CombineMeshes() {
		...
		sTarget.combinedGameOjects = new GameObject[meshFilters.Length];
		for (int i = 0; i < meshFilters.Length; i++) {
			sTarget.combinedGameOjects[i] = meshFilters[i].gameObject;
			...
			EditorUtility.DisplayProgressBar("Combining", "", (float)i/ meshFilters.Length);
		}
		...
		combo.GetComponent<MeshFilter>().sharedMesh = ms;//.CombineMeshes(combineInstances, false, false);
														 //combo.GetComponent<MeshFilter>().sharedMesh.Optimize();
		...
		if (meshRendererCombine == null) meshRendererCombine = sTarget.gameObject.AddComponent<MeshRenderer>();
		Material[] materialsArray = materials.ToArray(typeof(Material)) as Material[];
		meshRendererCombine.materials = materialsArray;
		sTarget.combined = combo.gameObject;
		combo.transform.parent = sTarget.transform;
		ToggleRenderers(false);

		SpriteRenderer sr = combo.transform.parent.GetChild(0).GetComponent<SpriteRenderer>();
		if(sr){
			combo.GetComponent<Renderer>().sortingOrder = sr.sortingOrder;
			combo.GetComponent<Renderer>().sortingLayerName = sr.sortingLayerName;
		}

		

		sTarget.vCount = ms.vertexCount;
